			...
			<div class="row">
				<div class="col-lg-12">
					...
					<h2 class="section-heading">
						My Music List:<br>MK Music
					</h2>
					<p class="lead">내 구매 내역 살펴보기:: 총 수량: ${totalNum }, 총 금액:
						${totalPrice }</p>
					<input type="button" value="처음으로" onclick="toList()"
						style="text-align: left;" class="btn btn-default">
					<table>
						<tr>
							<td>아티스트</td>
							<td>제목</td>
						...
						<c:forEach var="pList" items="${purchaseList }">
							<tr>
								<td>${pList.artist }</td>
								<td>${pList.title }</td>
								<td>${pList.quantity }</td>
								<td>${pList.amount }</td>
								<td>${pList.salesdate }</td>
							</tr>
						</c:forEach>
					</table>
				</div>
